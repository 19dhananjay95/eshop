...
def home(Request):
  data = Product.objects.all().order_by('id').reverse()[0:8]
...
def loginPage(Request):
  if(Request.method=="POST"):
    ...
    if(user is not None):
      ...
      messages.error(Request,"Invalid Userename or Password!!!")
...
@login_required(login_url='/login/')
def addToWishlist(Requset,pid):
  try:
    user = Buyer.objects.get(username=Requset.user.username)
